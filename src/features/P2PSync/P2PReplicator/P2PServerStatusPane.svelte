...
<div class="p2p-container">
    <div class="pane-header">
        ...
        <div class="pane-header-actions">
            <div class="remote-picker-wrap">
                ...
                <button class="icon-button" onclick={() => createAndSelectP2PRemote()} title="P2Pリモートを作成" aria-label="P2Pリモートを作成">
                    +
                </button>
            </div>
            <button
    ...
    {#if !canEditP2PSettings()}
        <p class="warning-line">P2P同期対象を変更するには、有効なP2Pリモート設定を選択してください。</p>
    {/if}
                        ...
                        <div class="peer-actions">
                            {#if isAccepted(peer)}
                                ...
                                <div class="decision-row watch-row">
                                    <span class="decision-label">監視</span>
                                    <button
                                        class="emoji-button {isWatching(peer.peerId) ? 'is-watching' : ''}"
                                        title={isWatching(peer.peerId) ? 'このピアを監視中です。クリックすると停止します' : 'このピアの変更を監視します'}
                                        aria-label={isWatching(peer.peerId) ? '監視を停止' : 'ピアを監視'}
                                    ...
                                    <span class="decision-label">同期</span>
                                    <button
                                        class="emoji-button {isSyncTarget(peer.name) ? 'is-watching' : ''}"
                                        title={isSyncTarget(peer.name) ? '同期対象です。クリックすると解除します' : '同期対象に設定します'}
                                        aria-label={isSyncTarget(peer.name) ? '同期対象を解除' : '同期対象に設定'}
                                        disabled={!canEditP2PSettings()}
                                ...
                                <div class="decision-row">
                                    <span class="decision-label">永続</span>
                                    <button
                                        class="emoji-button"
                                        title="永続的に承認"
                                        aria-label="永続的に承認"
                                ...
                                <div class="decision-row">
                                    <span class="decision-label">セッション</span>
                                    <button
                                        class="emoji-button"
                                        title="このセッションのみ承認"
