...
export function useInteractiveConflictResolutionFeature(
    ...
    let offPluginUnloaded: (() => void) | undefined;
    const dispose = () => {
        if (disposed) return;
        disposed = true;
        ...
        offPluginUnloaded?.();
        offPluginUnloaded = undefined;
        operations.dispose();
    };
    offConflictCancelled = services.context.events.onEvent(
    ...
    offPluginUnloaded = services.context.events.onceEvent(EVENT_PLUGIN_UNLOADED, dispose);
