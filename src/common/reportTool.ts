...
export async function generateReport(settings: ObsidianLiveSyncSettings, core: LiveSyncBaseCore) {
    ...
    if (settings.remoteType == REMOTE_COUCHDB) {
        try {
            ...
            responseConfig = {
                error: "リモートCouchDBからの情報取得に失敗しました。IBM Cloudantを使用している場合、これは通常の挙動です。",
            };
        }
    } else if (settings.remoteType == REMOTE_MINIO) {
        responseConfig = { error: "オブジェクトストレージ同期" };
        //
    }
    ...
    const redact = (source: string) => `${REDACTED}(${source.length}文字)`;
    ...
    pluginConfig.P2P_turnServers = `(${pluginConfig.P2P_turnServers.split(",").length}個のサーバーが設定されています)`;
    const endpoint = pluginConfig.endpoint;
    if (endpoint == "") {
        pluginConfig.endpoint = "未設定またはAWS";
    } else {
