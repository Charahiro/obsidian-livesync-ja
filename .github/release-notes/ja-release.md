...
- 設定項目の名称、説明、選択肢、およびプレースホルダーをCommonlibの定義から正しく日本語化
- 言語設定ダイアログの未展開プレースホルダーと、修復・高度な設定・パッチ各タブに残っていた英語を修正
- 競合解決、互換性レビュー、セットアップ、および復旧操作の通知・確認メッセージを日本語化
- パッチタブの「是正」で、設定名、説明、現在の制限日時、および適用ボタンが表示されるよう修正
