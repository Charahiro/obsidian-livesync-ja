...
コマンドパレットから `Issue作成用のデバッグ情報付き完全レポートを生成` コマンドを実行すると、レポートをクリップボードへコピーできます。

### ログはどこで確認できますか？

コマンドパレットの `Show log` からログペインを開けます。問題が発生している場合は、`General Setting` ペインで `Verbose Log` を有効にしてください。
`Issue作成用のデバッグ情報付き完全レポートを生成` コマンドには、デバッグに有用な直近1000行のログも含まれます。この完全レポートは verbose レベルのログを含むため、`Verbose Log` を有効にしていなくても詳細ログが入ります。

レポートを共有する前に、機密情報が含まれていないか必ず確認してください。
