...
#### TURN設定
設定キー: P2P_managedType

**手動**では既存のTURNサーバー設定を使用し、**管理型（Cloudflare）**ではTURNキーIDとTURNキーのAPIトークンを指定します。APIトークンはプロファイルに保存され、セットアップURIやQRコードによる共有にも含まれます。発行された一時的な認証情報はメモリ内にのみ保持されます。レポートではプロバイダー設定が伏せられます。

#### TURNキーIDとTURNキーのAPIトークン
設定キー: P2P_managedId, P2P_managedToken

**管理型（Cloudflare）**を選択した場合に表示されます。TURNキーのIDと、そのキー専用のAPIトークンを入力してください。トークン欄の入力内容は伏せられます。アカウントID、独自のエンドポイント、更新間隔の指定は不要です。
...
#### P2Pメッセージサイズ
設定キー: P2P_maxWirePayloadBytes

このプロファイル設定は、Trysteroが独自のフレーミングを適用する前の、送信するCommonlib RPCメッセージごとのサイズを制限します。Vaultのチャンクサイズ、IP MTU、SCTPフラグメントサイズではありません。利用できるプリセットは、**標準**（15,360バイト）、**縮小**（2,048バイト）、**保守的**（1,024バイト）、および**最大の互換性**（800バイト）です。小さい値ほど、大きなWebRTCメッセージが失われる経路での互換性を高められますが、転送速度は低下します。

送信側のデバイスが送信メッセージのサイズを制御します。制約のある経路で送信する可能性があるすべてのデバイスで、同じ保守的なプリセットを選択してください。このキーがない既存プロファイルでは**標準**が使われます。P2P接続文字列と暗号化されたセットアップURIには、選択したプリセットが保持されます。

#### 接続経路
設定キー: P2P_connectionPath

**自動**はWebRTCに直接接続またはTURN中継の利用可能な経路を選択させる既定値です。**TURNリレーのみ**は`iceTransportPolicy: 'relay'`を強制し、有効な手動TURN URLまたはTURN認証情報の取得元を設定したプロファイルで選択できます。TURNリレーのみを選択中に手動TURN設定を削除した場合は、接続経路を**自動**に戻して通知します。選択した認証情報の取得元から認証情報を取得できない場合は、接続を開始せず、接続経路も変更しません。

この選択はP2Pプロファイルに属し、P2P接続文字列と暗号化されたセットアップURIに保持されます。同じグループIDと認証情報を使用する別のプロファイルで異なる互換性設定を選択できますが、有効になるのは選択中のP2Pプロファイルだけです。
