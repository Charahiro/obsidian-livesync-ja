...
**自動**はWebRTCに直接接続またはTURN中継の利用可能な経路を選択させる既定値です。**TURNリレーのみ**は`iceTransportPolicy: 'relay'`を強制し、有効な`turn:`または`turns:` URLが少なくとも1つ含まれるプロファイルでのみ選択できます。TURN URLをすべて削除した場合は、接続経路を**自動**に戻して通知します。
